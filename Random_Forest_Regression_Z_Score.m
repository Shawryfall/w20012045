...
% Test the random forest regression model using the testing subset
...
fprintf('Random Forest Regression(Z-score scaling) Mean Squared Error: %.2f\n', mse_rf);

% Calculate and display the Mean Absolute Error (MAE)
mae_rf = mean(abs(y_test - y_pred_rf));
fprintf('Random Forest Regression(Z-score scaling) Mean Absolute Error: %.2f\n', mae_rf);

% Calculate and display the Pearson correlation coefficient (r value)
r_value = corr(y_test, y_pred_rf);
fprintf('Random Forest Regression(Z-score scaling) Pearson correlation coefficient (r value): %.2f\n', r_value);

% Calculate and display the Residual Standard Error (RSE)
n = length(y_test);
p = length(mdl_rf.PredictorNames); % Number of predictor variables
RSS = sum((y_test - y_pred_rf).^2);
RSE = sqrt(RSS / (n - p - 1));
fprintf('Random Forest Regression(Z-score scaling) Residual Standard Error (RSE): %.2f\n', RSE);
