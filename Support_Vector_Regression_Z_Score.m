...
mdl_svr = fitrsvm(X_train, y_train, 'KernelFunction', 'gaussian', 'KernelScale', 'auto', 'Standardize', false);

% Test the support vector regression model using the testing subset
...
fprintf('Support Vector Regression(Z-score scaling) Mean Squared Error: %.2f\n', mse_svr);

% Calculate and display the Mean Absolute Error (MAE)
mae_svr = mean(abs(y_test - y_pred_svr));
fprintf('Support Vector Regression(Z-score scaling) Mean Absolute Error: %.2f\n', mae_svr);

% Calculate and display the Pearson correlation coefficient (r value)
r_value = corr(y_test, y_pred_svr);
fprintf('Support Vector Regression(Z-score scaling) Pearson correlation coefficient (r value): %.2f\n', r_value);
...
RSE = sqrt(RSS / (n - p - 1));
fprintf('Support Vector Regression(Z-score scaling) Residual Standard Error (RSE): %.2f\n', RSE);
